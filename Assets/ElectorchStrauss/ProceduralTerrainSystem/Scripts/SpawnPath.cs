...
namespace ElectorchStrauss.ProceduralTerrainSystem.Scripts
{
    public class SpawnPath : MonoBehaviour
    {
        ...
        public bool resetPath, addPath,curvePath;
        public bool spawnIt;
        private void Start()
        {
        ...
        public void RoundSmoothPath()
        {
            
        }

        public void SubdividePath()
        {
            //takes the end start point and find the middle of the path
            //gets second half of child and send them in another path
            
            //add the middlePath to an array of subdivision
        }
            ...
            if (first && second && !third)
            {
                ...
                for (int i = 0; i < pathPoints.Length; i++)
                {
                        ...
                        new Vector3(skyStart.x, skyStart.y + 100, skyStart.z), out RaycastHit hit))
                    {
                        if (curvePath)
                        {
                            pathPoints[i] = new Vector3(hit.point.x+Random.Range(-20f,20f), hit.point.y + height / Random.Range(0f,8f), hit.point.z+Random.Range(-20f,20f));
                        }
                        else
                        {
